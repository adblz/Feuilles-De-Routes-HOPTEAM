...
export function sauvegarderHistorique(mode) {
    try {
        ...
        memoriserValeurs(data.elements);

        // Une seule entrée par date — écrase l'existante si même jour
        const histaFiltered = histo.filter(e => e.date !== data.date);
        histaFiltered.unshift({
            id:              Date.now(),
            date:            data.date,
            tech:            data.tech,
        ...
        if (histaFiltered.length > MAX_HISTO) histaFiltered.splice(MAX_HISTO);
        localStorage.setItem('fdr_historique', JSON.stringify(histaFiltered));
    } catch (e) {}
}
